...
namespace led {
    ...
    // Immutable variables
    const uint8_t INPUT_0_PIN = 34;
    const uint8_t INPUT_1_PIN = 35;
    const uint8_t INPUT_2_PIN = 32;

    const uint8_t LED_0_PIN = 23;
    const uint8_t LED_1_PIN = 22;
    const uint8_t LED_2_PIN = 18;

    const uint8_t LED_0_PWM_CHAN = 0;
    const uint8_t LED_1_PWM_CHAN = 1;
    const uint8_t LED_2_PWM_CHAN = 2;
    ...
    class LEDHelper {
        public:
            static std::map<int, LEDOutput> determineOutput(std::map<int, LEDStateMachine>& fullConfig, std::map<int, bool> inputs);
    };
}